...
module ctrl (
    ...
    output reg         use_pc_d      // EX1 的 alu_a 选 PC（AUIPC/Branch/JAL）—预译码，移出EX1关键路径
);
  ...
  always @(*) begin
    ...
    case (opcode)
      ...
      7'b001_0011: begin
        ...
        alu_src_d = 1'b1;
        rs1_en_d  = 1'b1;
        if (funct3 == 3'b101 && funct7 == 7'b0010100 && d_instr[24:20] == 5'b00111) begin
          alu_op_d = `ALU_BEXT3;   // orc.b(现场添加)
        end else
        case (funct3)
      ...
      // ══ B 扩展现场槽位 ═══════════════════════════════════════════
      // 决赛现场加一条 zba/zbb/zbc/zbs/zbkb/zbkx 指令。逐条的机器码、ALU
      // 表达式、译码分支见 bext/paste_lib.md(36 条全部 ModelSim 验证过)。
      //
      //  两个会让你【静默答错】的坑:
      //   1. R 型:上面的「原 RV32I R 型译码」只看 f7b5(= d_instr[30]),
      //      其余 funct7 位全忽略;slli 更是完全不查 funct7。新指令必须插一个
      //      【精确 funct7】的分支,插在 Zba 分支之后、RV32I 的 else 之前。
      //      否则它会被译码成一条现成的 RV32I 指令,不报错、不跑飞,只是算错。
      //   2. I 型:d_instr[31:25] 对 I 型来说【是立即数高位】,不是 funct7。
      //      守卫必须先锁 funct3 ∈ {001,101},否则 addi rd,rs1,0x600 的
      //      instr[31:25] 恰好等于 rori 的 funct7,会被误判成 rori。
      //
      // andn/orn/xnor 的 funct7 = 0100000,与 and/or/xor 落在同一条 arm,
      // 只能改旧的三元判断:  3'b111: alu_op_d = f7b5 ? `ALU_BEXT3 : `ALU_AND;
      // (RV32I 的 and 恒为 funct7=0000000 -> f7b5=0 -> 行为逐位不变)
